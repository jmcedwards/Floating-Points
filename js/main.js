...
var numParticles = 1;//perfect sq
var particlesTextureDim = 1;//sqrt(numParticles)
...
function render(){
    ...
    if (!paused) {
        ...
        if (!mouseout && mouseEnable){
            ...
            // GPU.setUniformForProgram("force", "u_mouseEnable", 1.0, "1f");
            // GPU.setUniformForProgram("force", "u_mouseCoord", [200,200], "2f");
            // GPU.setUniformForProgram("force", "u_mouseDir", [3*(200)*scale,
            //     3*(200)*scale], "2f");
        }
        ...
        if (!mouseout && mouseEnable){
            ...
            // GPU.setUniformForProgram("addMaterial", "u_mouseEnable", 1.0, "1f");
            // GPU.setUniformForProgram("addMaterial", "u_mouseCoord", [200,200], "2f");
            // GPU.setUniformForProgram("addMaterial", "u_mouseLength", Math.sqrt(Math.pow(3*(200),2)
            //     +Math.pow(3*(200),2)), "1f");
        }
...
function onMouseMove(e){
    ...
    // toggles mouseEnable when the mouse is moving
    mouseEnable = true;
    clearTimeout(timeout);
    timeout = setTimeout(function(){mouseEnable = false;}, 50);

    var padding = 10;
    lastMouseCoordinates = mouseCoordinates;
    
    // enables fluid to correctly react when mouse moves in and out the window
    var x = e.clientX;
    var y = e.clientY;
    if (x<padding){
        x = padding;
        ...
        lastMouseCoordinates = [x, actualHeight-y];
    }
    ...
    if (y<padding){
        ...
        if (x>actualHeight-padding) x = actualHeight-padding;
        lastMouseCoordinates = [x, actualHeight-y];
    }
...
// keep these or code won't run
function onMouseDown(){
}
function onMouseUp(){
}

// stops movement when the mouse leaves the window
$(document).mouseleave(function () {
    lastMouseCoordinates = [0,0];
    mouseout = true;
});
